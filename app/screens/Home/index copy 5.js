...
class HomeScreen extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      dataMultiOwner: [],
      dataUserRequest: [],
      dataFinalBindingMultiOwner: [],
      dataUserApproveBinding: [],
    };
  ...
  //========MULTI OWNER=========//
  doCheckUserAcctBindingMultiOwner = (phonenumberUnOwner, phonenumberOwner) => {
    let params = {
    ...
    fetch(
      global.serverurl +
        ...
        '/app_check_userbinding_unowner1.php',
      {
        method: 'POST',
        headers: {
      ...
      .then((response) => {
        ...
        if (response.status === 'OK') {
          if (response.UserAcct.length > 0) {
            console.log(response.UserAcct[0], response.UserRequest[0]);
            this.doBindingAccountUnOwnerMulti(
              response.UserAcct[0],
              response.UserRequest[0],
            );
            // let dataMultiOwnerArr = this.state.dataMultiOwner;
            // dataMultiOwnerArr.push(
            //   response.UserAcct[0],
            //   response.UserRequest[0],
            // );
            // this.setState({
            //   dataMultiOwner: dataMultiOwnerArr,
            // });

            // let dataUserRequestArr = this.state.dataUserRequest;
            // dataUserRequestArr.push(response.UserRequest[0]);
            // this.setState({
            //   dataUserRequest: dataUserRequestArr,
            // });

            // console.log(dataMultiOwnerArr);
            // console.log(dataUserRequestArr);
            // this.storeDataUser(
            //   'Modernland-Account',
            //   JSON.stringify(dataMultiOwnerArr),
            // );

            // if (dataMultiOwnerArr.length > 0) {
            //   dataMultiOwnerArr.forEach((num1, index) => {
            //     const num2 = dataUserRequestArr[index];
            //     console.log(num1, num2);
            //     console.log(index);
            //     // if (index > 0) {
            //     this.doBindingAccountUnOwnerMulti(num1, num2);
            //     // }
            //   });
            // }
          }
  ...
  doBindingAccountUnOwnerMulti = (AccountBindOwner, UserRequestUnit) => {
    var accountBindOwner = AccountBindOwner;
    var userRequestUnit = UserRequestUnit;
    console.log(accountBindOwner);
    console.log(JSON.stringify(accountBindOwner));
    fetch(
      ...
      .then((response) => {
        ...
        if (response.status === 'OK') {
          if (response.status === 'Error') {
            ...
            console.log(
              'Hasil dari response binding owner multi',
              response.data,
            );
            console.log('unit yg akan di ambil owner multi', userRequestUnit);
            let dataAwal = response.data.filter(
              (item) => item.DebtorAcct === userRequestUnit.debtoracct,
            );
            console.log(dataAwal[0]);
            let dataFinalMulti = this.state.dataFinalBindingMultiOwner;
            dataFinalMulti.push(dataAwal[0]);
            this.setState({
              dataFinalBindingMultiOwner: dataFinalMulti,
            });

            console.log('Final Data Multi Owner', dataFinalMulti);

            var resultMulti = dataFinalMulti.reduce((unique, o) => {
              if (
                !unique.some(
                  (obj) =>
                    obj.DebtorAcct === o.DebtorAcct &&
                    obj.LotNo === o.LotNo &&
                    obj.BisnisId === o.BisnisId,
                )
              ) {
                unique.push(o);
              }
            ...
            this.setState({
              dataUserApproveBinding: resultMulti,
            });
            console.log('token', payload);
            console.log('sending data bind owner', resultMulti);

            this.storeBinding(
              'smart-app-id-binding',
              JSON.stringify(resultMulti),
            );
  ...
  doCheckUserAcctBinding = (phone) => {
    ...
    fetch(
      global.serverurl + global.webserviceurl + '/app_check_userbinding.php',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8',
  ...
  userAkun = () => {
    this._retrieveData('Modernland-Account').then((info) => {
      console.log(info);
      if (
        this.state.userLogin.MultiOwner.length > 1 &&
        this.state.userLogin.typeRequestOwner === 2
      ) {
        if (info !== null) {
          console.log('linkAccount', JSON.parse(info));
          this.setState({linkAccountMulti: JSON.parse(info)});
        }
      } else {
        if (info !== null) {
  ...
  getTokenNewMultiOwner = () => {
    this._retrieveData('smart-app-id-token').then((info) => {
      ...
      if (
        ...
        this.state.linkAccountMulti.map((v) => {
          fetch(
            global.serverurl +
              global.webserviceurl +
    ...
    this._retrieveData('smart-app-id-login').then((info) => {
      ...
      if (info !== null) {
        ...
        if (info.statusownerid === 1) {
          ...
          info.statusownerid === 2 &&
          info.phonenumberOwner !== '' &&
          info.typeRequestOwner === 1
        ) {
          ...
          info.statusownerid === 2 &&
          info.typeRequestOwner === 2 &&
          info.MultiOwner.length > 1
        ) {
          console.log('Multi Owner');
          info.MultiOwner.map((v, i) => {
            this.doCheckUserAcctBindingMultiOwner(
              info.phonenumber,
              v.phonenumberOwner,
            );
          });
          ...
          console.log('Multi Owner but one owner');
  ...
  receivePost(param) {
    ...
    if (jsonParam.code === undefined) {
      ...
      } else if (jsonParam.code === 'OpenAlert-Binding') {
        Alert.alert(
          'Warning',
          'Your account have not binded yet',
          [
            {
              text: this.globallang.ok,
              onPress: () => console.log('OK Pressed'),
